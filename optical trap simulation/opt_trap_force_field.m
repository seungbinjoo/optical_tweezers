...
mu = [0, 0]; % Mean
sigma = [1, 0; 0, 1] * 2.2e-6; % Covariance matrix
...
% Plot the Gaussian
z_shift = 4 * 1e-3 + f;
gain = 1.7; % extra factor multipled to gaussian so the value heights are visible in plot

surf(x, y, (relative_intensity * 1e-3 * gain) - z_shift,'FaceAlpha', 1);
...
zlim([-z_shift, 1 * 1e-3]); % Z AXES LIMITS!
...
%% Plot sphere representing particle --> although it won't be visible in zoomed out plot
% Plot spherical particle
...
figure;
surf(X2+sphere_center(1),Y2+sphere_center(2),Z2+sphere_center(3),'FaceColor',[1 1 0],'FaceAlpha', 0.3)
...
title('Transverse Plane (NA = 1.1)','FontSize',18);
xlabel('X (m)','FontSize',16);
ylabel('Y (m)','FontSize',16);
xlim([-1.6 1.6]*R)
ylim([-1.6 1.6]*R)
set(gca, 'FontSize', 16) % grid ticks
axis equal
hold on
...
plot(x_circle, y_circle,'k', 'LineWidth', 2);  % Plot the circle
xlim([-1.6 1.6]*R)
ylim([-1.6 1.6]*R)
hold on

% Calculate gradient of the force field --> trapping stiffness
% [dx, dy] = gradient(sqrt(f_x_2D_tran.^2+f_y_2D_tran.^2), x, y);

% Plot contour plot of the gradient --> trapping stiffness
% contour(X_tran', Y_tran', sqrt(dx.^2 + dy.^2),'LineWidth', 0.1);

%% Plot trapping force along x axis

f_x_2D_tran = f_x_2D_tran';
force_x = f_x_2D_tran(round(size(f_x_2D_tran,1)/2),:);

hold off
figure;
plot(x,force_x,'LineWidth',1)
xlabel('X (m)');
ylabel('Trapping force (N)');
title('Trapping force along X axis');
xlim([-1.6 1.6]*R)
...
% Plot 2D force vector field
% Transpose the 2D matrices (idk exactly why but this bug took me ages to solve)
quiver(Z_long',X_long',f_z_2D_long,f_x_2D_long,'LineWidth',2)
title('Longitudinal Plane (NA = 1.1)','FontSize',18);
...
% Calculate gradient of the force field  --> trapping stiffness
% [dx, dz] = gradient(sqrt(f_z_2D_long.^2+f_x_2D_long.^2), x, z);

% Plot contour plot of the gradient --> trapping stiffness
% contour(X_long', Z_long', sqrt(dx.^2 + dz.^2),'LineWidth', 0.1);

%% Trapping force along z axis

force_z = f_z_2D_long(round(size(f_z_2D_long,1)/2),:);

hold off
figure;
plot(z,force_z,'LineWidth',1)
xlabel('Z (m)');
ylabel('Trapping force (N)');
title('Trapping force along Z axis')
xlim([-1.6 1.6]*R)

% Plotting both force plots on same graph
figure;
plot(x,force_x,'LineWidth',1)
xlabel('X or Z (m)','FontSize',16);
ylabel('Trapping force (N)','FontSize',16);
title('Trapping force along X and Z axis','FontSize',18);
xlim([-2.4 2.4]*R)
set(gca, 'FontSize', 16) % grid ticks
hold on
plot(z,force_z,'LineWidth',1)
legend('Trapping force in x direction', 'Trapping force in z direction','FontSize',16)