...
% Find the indices corresponding to X_translation in x_target
[~, x_indices] = arrayfun(@(x) min(abs(x_target - x)), X_translation);

% Find the indices corresponding to Y_translation in y_target
[~, y_indices] = arrayfun(@(y) min(abs(y_target - y)), Y_translation);
...
% Storing standard percentage error
sigma = zeros(n_iterations,1);
...
for i = 1:n_iterations
    ...
    % Calculate MSE between target and reconstructed distributions
    % error = (sum((abs(E_target(:)) - abs(E_focus(:))).^2))/(resolution*resolution);
    % error_vector(i) = error;

    % Extract the normalised intensities for each trap and put them in each row
    % Each row represents a trap
    % Extract a 20 by 20 grid of intensities around each trap
    side_length = 21;
    nth_trap_intensity = zeros(numel(X_translation),side_length^2);
    for k = 1:numel(X_translation)
        center_point = [x_indices(k) y_indices(k)];
        submatrix = extractSubmatrix(normalized_E_focus, center_point, side_length);
        nth_trap_intensity(k,:) = submatrix(:);
    end
    nth_trap_mean = mean(nth_trap_intensity,2);
    
    I_tot = sum(nth_trap_mean,'all');
    I_mean = I_tot/9;
    sigma(i,1) = (sqrt(sum((nth_trap_mean-I_mean).^2,'all')))/(I_mean*9) * 100;
end
...
title('Error between target distribution & reconstructed focal plane distribution')
xlabel('Iterations of GS algorithm')
ylabel('Standard percentage error')
...
function submatrix = extractSubmatrix(matrix, center_point, submatrix_size)
    % Ensure submatrix size is odd for symmetric extraction
    if mod(submatrix_size, 2) == 0
        error('Submatrix size must be odd for symmetric extraction.');
    end
    
    % Get the indices of the center point
    center_row = center_point(1);
    center_col = center_point(2);
    
    % Calculate the offset for symmetric extraction
    offset = floor(submatrix_size / 2);
    
    % Extract the submatrix
    start_row = max(1, center_row - offset);
    end_row = min(size(matrix, 1), center_row + offset);
    start_col = max(1, center_col - offset);
    end_col = min(size(matrix, 2), center_col + offset);
